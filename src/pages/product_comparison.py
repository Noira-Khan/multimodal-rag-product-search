...
st.caption("Compare two fashion products using AI.")
...
product_names = sorted(engine.product_names())
...
if compare_button:
    if product_a == product_b:
        ...
        with st.spinner("Comparing products..."):
            comparison = engine.compare_products(product_a, product_b)

        if comparison is None:
            st.error("Unable to compare these products.")
        else:
            analytics.log_comparison(product_a, product_b)
            ...
            with st.spinner("Gemini is analysing both products..."):
                ai_response = engine.ai_compare_products(product_a, product_b)
...
if st.session_state.comparison_history:
    for i, item in enumerate(
        ...
        with st.expander(f"Comparison {i}"):
            st.write("**Product A:**", item["product_a"])
            st.write("**Product B:**", item["product_b"])
...
if st.button(
    ...
    st.session_state.comparison_history = []
    st.success("Comparison history cleared.")
    st.rerun()
...
with st.expander("⚙ Engine Status"):
    ...
    st.write("Products:", len(product_names))
    st.write("Previous Comparisons:", len(st.session_state.comparison_history))
